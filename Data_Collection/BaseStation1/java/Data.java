...
class Data {
    ...
    int getTemp(int nodeId, int x) {
    if (nodeId >= nodes.length || nodes[nodeId] == null) {
    		//System.out.println("Data   missing data for nodeId: " + nodeId);
        return -1;
    }
    return nodes[nodeId].getTemp(x);
    }

    int getHumid(int nodeId, int x) {
    if (nodeId >= nodes.length || nodes[nodeId] == null)
        return -1;
    return nodes[nodeId].getHumid(x);
    }

    int getLight(int nodeId, int x) {
    if (nodeId >= nodes.length || nodes[nodeId] == null)
        return -1;
    return nodes[nodeId].getLight(x);
    }
